/* ═══════════════════════════════════════════════════════════════
   ...
   5. Parallax do hero
   6. Abas do cardápio com marcador deslizante
   7. Galeria: lightbox
...
(function () {
  ...
  /* ─── 6. ABAS DO CARDÁPIO ───────────────────────────────── */
  var abas = $('#abas');
  var marcador = $('#abasMarcador');
  var grupos = $$('.cardapio__grupo');

  function posicionarMarcador(btn) {
    if (!marcador || !btn) return;
    marcador.style.left = btn.offsetLeft + 'px';
    marcador.style.width = btn.offsetWidth + 'px';
  }
  ...
  if (abas && grupos.length) {
    ...
    abas.addEventListener('click', function (e) {
      ...
      posicionarMarcador(btn);

      // reanima os cards da categoria que acabou de aparecer
      $$('.prato', $('[data-grupo="' + btn.dataset.aba + '"]')).forEach(function (card, i) {
    ...
    posicionarMarcador($('.abas__btn.ativo', abas));
