...
import { chamadaProjetos, linkTexto, planos } from "@/content/agencia";
import { demos } from "@/content/demos";

/* Luminância relativa (WCAG) para decidir texto claro ou escuro
   sobre a cor do cartão. */
function ehClaro(hex: string) {
  const n = hex.replace("#", "");
  const canais = [0, 2, 4].map((i) => {
    const v = parseInt(n.slice(i, i + 2), 16) / 255;
    return v <= 0.03928 ? v / 12.92 : Math.pow((v + 0.055) / 1.055, 2.4);
  });
  return 0.2126 * canais[0] + 0.7152 * canais[1] + 0.0722 * canais[2] > 0.4;
}
  ...
  return (
    <Secao id="projetos" tom="poco">
      <CabecaSecao
        indice="03"
        etiqueta="Work"
        titulo="Three restaurants, three different jobs to do"
        texto="Each one is built around a different action: reserve, order, call. Open one on your phone and scroll to the end — it's the working site, not a picture of one."
      />

      <div className="mt-16 space-y-5 sm:mt-20">
        {demos.map((d, i) => {
          /* Se o cartão é claro ou escuro sai da LUMINÂNCIA da própria
             cor, não de uma lista de slugs: troque `corCard` em
             content/demos.ts e o texto se inverte sozinho. */
          const claro = ehClaro(d.corCard);
          const plano = planos.lista.find((p) => p.slug === d.slug);
          const corTexto = claro ? "#221E1C" : "#EDEAE4";
          const corApoio = claro ? "rgba(34,30,28,0.68)" : "rgba(237,234,228,0.62)";
              ...
              >
                <div className="flex flex-col p-8 sm:p-12">
                  ...
                  <div className="mt-auto flex flex-wrap items-center gap-x-6 gap-y-4 pt-10">
                    ...
                    <Link
                      ...
                      style={{ backgroundColor: d.corTexto, color: ehClaro(d.corTexto) ? "#0B0B0C" : "#F7F1E3" }}
                    >
                      Open the site
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {/* O dono do restaurante não liga para nome de fonte.
                ...
                <div className="relative flex items-end justify-center px-8 pb-0 lg:px-0">
                  ...
                  >
                    <Image
                      src={d.mockup}
                      alt={`The ${d.nome} website shown on a phone`}
                      width={420}
                      height={760}
                      loading="lazy"
      ...
      <Revelar atraso={0.1}>
        <p className="mt-10 text-[0.78rem] text-osso-3">
          Fictional restaurants, built for demonstration. Nothing here matches a real business.
        </p>
      </Revelar>
      ...
      <Revelar atraso={0.14}>
        <div className="mt-14 flex flex-col items-start gap-7 border-t border-fio-forte pt-12 lg:flex-row lg:items-center lg:justify-between">
          ...
          <Botao
            href={linkTexto(chamadaProjetos.mensagemTexto)}
            variante="claro"
            tamanho="lg"
            externo
            className="shrink-0"
