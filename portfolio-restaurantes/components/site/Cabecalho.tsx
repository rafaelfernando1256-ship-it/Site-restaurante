...
import { agencia, hero, linkTexto } from "@/content/agencia";

const links = [
  { href: "/#maquina", rotulo: "What's included" },
  { href: "/#projetos", rotulo: "Work" },
  { href: "/#conta", rotulo: "The math" },
    ...
    >
      <div className="relative z-50 mx-auto flex max-w-6xl items-center justify-between gap-6 px-5 py-4 sm:px-8">
        ...
        <nav className="hidden items-center gap-8 lg:flex" aria-label="Main navigation">
          {links.map((l) => (
            <Link
              key={l.href}
              href={l.href}
              className="relative flex min-h-[40px] items-center text-[0.88rem] text-osso-2 transition-colors duration-400 hover:text-osso after:absolute after:bottom-[5px] after:left-0 after:h-px after:w-full after:origin-right after:scale-x-0 after:bg-osso after:transition-transform after:duration-500 after:ease-[var(--ease-saida)] hover:after:origin-left hover:after:scale-x-100"
            >
              {l.rotulo}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-3">
          ...
          <Botao
            href={linkTexto(hero.mensagemTexto)}
            variante="contorno"
            externo
            className="max-sm:hidden"
          >
            <IconeTexto className="h-4 w-4" />
            Show me mine
          </Botao>
